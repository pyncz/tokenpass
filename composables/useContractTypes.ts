...
export function useContractTypes(
  ...
  const interfacesMap = computedAsync<InterfacesMap>(
    async () => {
      ...
      if (isContract.value && unref(provider)) {
        // It's a contract!
        try {
          ...
          const maybeErc165Contract = new Contract(unref(address), ERC165Abi, unref(provider))
          ...
          const maybeErc20Contract = new Contract(unref(address), ERC20Abi, unref(provider))
